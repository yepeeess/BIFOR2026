...
const defaultQuizzes = [
  ...
  },
  {
    id: 'q4',
    title: 'Trivia BIFOR',
    desc: 'Pon a prueba tus conocimientos sobre innovación y el evento',
    preguntas: [
      { pregunta: '¿Cuál es una buena forma de convertir una idea en innovación?', opciones:['Ignorar a los usuarios','Probar, aprender y mejorar','Copiar sin adaptar','Evitar medir resultados'], correcta:1 },
      { pregunta: '¿Qué ayuda a que una solución sea sostenible?', opciones:['Generar más residuos','Usar recursos sin límite','Crear valor social, ambiental y económico','Pensar solo a corto plazo'], correcta:2 },
      { pregunta: '¿Cuál es el propósito del ranking BIFOR?', opciones:['Castigar errores','Motivar la participación y el aprendizaje','Reemplazar el carnet','Registrar contraseñas'], correcta:1 },
      { pregunta: '¿Qué ciudad recibe BIFOR 2026?', opciones:['Pereira','Manizales','Armenia','Ibagué'], correcta:1 }
    ]
  }
];
...
function load(){
  ...
  // Incorporar la trivia al almacenamiento de quienes ya usaban la versión anterior.
  if(!state.quizzes.some(q=>q.id==='q4')) state.quizzes.push(JSON.parse(JSON.stringify(defaultQuizzes.find(q=>q.id==='q4'))));
...
function renderJuegos(){
  ...
  stopSnake();
  quizArea.classList.add('hidden'); snakeArea?.classList.add('hidden'); result.classList.add('hidden'); lista.classList.remove('hidden');
  if(!state.currentUser){
  ...
  const snakeCard = `
    <div class="juego-card snake-game-card">
      <div class="juego-watermark">NUEVO</div>
      <h4>🐍 Serpiente BIFOR</h4>
      <p>Recoge los puntos, evita chocar y consigue la mayor puntuación posible.</p>
      <div class="juego-meta"><span>⌨️ Flechas</span><span>📱 Controles táctiles</span><span>🏆 +50 pts</span></div>
      <button class="btn-primary" onclick="startSnake()">Jugar Serpiente →</button>
    </div>`;
  lista.innerHTML = snakeCard + state.quizzes.map(q=>`
...
function startSnake(){
  ...
  document.getElementById('snakeArea').classList.remove('hidden');
  document.body.classList.add('playing-snake');
  state.snake={ cells:20, body:[{x:10,y:10},{x:9,y:10},{x:8,y:10}], direction:{x:1,y:0}, nextDirection:{x:1,y:0}, food:{x:15,y:10}, score:0 };
  document.getElementById('snakeScore').textContent='0';
  setPlaying(state.currentUser.email,true,'Serpiente BIFOR');
  touchUser(state.currentUser.email);
  drawSnake();
  canvas.focus({preventScroll:true});
  clearInterval(state.snakeTimer);
  state.snakeTimer=setInterval(moveSnake,115);
}

function setSnakeDirection(direction){
  const snake=state.snake; if(!snake) return;
  const directions={up:{x:0,y:-1},down:{x:0,y:1},left:{x:-1,y:0},right:{x:1,y:0}};
  const next=directions[direction]; if(!next) return;
  if(next.x===-snake.direction.x && next.y===-snake.direction.y) return;
...
function moveSnake(){
  ...
  const hitWall=head.x<0 || head.x>=snake.cells || head.y<0 || head.y>=snake.cells;
  const hitBody=snake.body.some(p=>p.x===head.x && p.y===head.y);
  if(hitWall || hitBody){ finishSnake(); return; }
  snake.body.unshift(head);
  if(head.x===snake.food.x && head.y===snake.food.y){
    snake.score+=50;
    document.getElementById('snakeScore').textContent=snake.score;
    playSfx('correct');
    do { snake.food={x:Math.floor(Math.random()*snake.cells),y:Math.floor(Math.random()*snake.cells)}; }
...
function drawSnake(){
  const canvas=document.getElementById('snakeCanvas'), snake=state.snake;
  if(!canvas || !snake) return;
  const ctx=canvas.getContext('2d'), size=canvas.width/snake.cells;
  const rounded=(x,y,w,h,r)=>{
    ctx.beginPath(); ctx.moveTo(x+r,y); ctx.arcTo(x+w,y,x+w,y+h,r); ctx.arcTo(x+w,y+h,x,y+h,r); ctx.arcTo(x,y+h,x,y,r); ctx.arcTo(x,y,x+w,y,r); ctx.closePath();
  };
  const gradient=ctx.createLinearGradient(0,0,canvas.width,canvas.height);
  gradient.addColorStop(0,'#0d2920'); gradient.addColorStop(1,'#04100c');
  ctx.fillStyle=gradient; ctx.fillRect(0,0,canvas.width,canvas.height);
  ctx.strokeStyle='rgba(142,209,82,.09)'; ctx.lineWidth=1;
  for(let i=0;i<=snake.cells;i++){
  ...
  // Energía que debe recoger el jugador.
  const fx=(snake.food.x+.5)*size, fy=(snake.food.y+.5)*size;
  const glow=ctx.createRadialGradient(fx,fy,1,fx,fy,size*.7);
  glow.addColorStop(0,'rgba(15,185,160,.8)'); glow.addColorStop(1,'rgba(15,185,160,0)');
  ctx.fillStyle=glow; ctx.fillRect(fx-size*.7,fy-size*.7,size*1.4,size*1.4);
  ctx.fillStyle='#19dec1'; ctx.beginPath(); ctx.arc(fx,fy,size*.24,0,Math.PI*2); ctx.fill();
  ctx.fillStyle='#d9fff7'; ctx.beginPath(); ctx.arc(fx-size*.07,fy-size*.08,size*.07,0,Math.PI*2); ctx.fill();

  snake.body.slice().reverse().forEach((p,reverseIndex)=>{
    const i=snake.body.length-1-reverseIndex;
    const inset=i===0 ? 1 : 2;
    const x=p.x*size+inset, y=p.y*size+inset, side=size-inset*2;
    const bodyGradient=ctx.createLinearGradient(x,y,x+side,y+side);
    bodyGradient.addColorStop(0,i===0?'#d9ff8d':'#9ee85c');
    bodyGradient.addColorStop(1,i===0?'#72be3c':'#4d9c35');
    rounded(x,y,side,side,Math.max(4,size*.24));
    ctx.fillStyle=bodyGradient; ctx.fill();
    if(i>0){
      ctx.fillStyle='rgba(14,59,46,.42)'; ctx.beginPath(); ctx.arc(x+side*.5,y+side*.5,side*.15,0,Math.PI*2); ctx.fill();
    }
  });

  // Cara direccional de la serpiente: ojos y lengua apuntan hacia su movimiento.
  const head=snake.body[0], hx=head.x*size, hy=head.y*size;
  const dir=snake.direction;
  const perpendicular={x:-dir.y,y:dir.x};
  const eyeBase={x:hx+size*.5+dir.x*size*.16,y:hy+size*.5+dir.y*size*.16};
  [-1,1].forEach(side=>{
    const ex=eyeBase.x+perpendicular.x*size*.19*side, ey=eyeBase.y+perpendicular.y*size*.19*side;
    ctx.fillStyle='#102313'; ctx.beginPath(); ctx.arc(ex,ey,size*.105,0,Math.PI*2); ctx.fill();
    ctx.fillStyle='#fff'; ctx.beginPath(); ctx.arc(ex-dir.x*size*.022,ey-dir.y*size*.022,size*.035,0,Math.PI*2); ctx.fill();
  });
  ctx.strokeStyle='#ff6b75'; ctx.lineWidth=2; ctx.lineCap='round';
  ctx.beginPath();
  ctx.moveTo(hx+size*.5+dir.x*size*.42,hy+size*.5+dir.y*size*.42);
  ctx.lineTo(hx+size*.5+dir.x*size*.67,hy+size*.5+dir.y*size*.67);
  ctx.stroke();
}

function stopSnake(){
  if(state.snakeTimer){ clearInterval(state.snakeTimer); state.snakeTimer=null; }
  state.snake=null;
  document.body.classList.remove('playing-snake');
}

function finishSnake(){
  const score=state.snake?.score||0;
  stopSnake();
  document.getElementById('snakeArea')?.classList.add('hidden');
  if(state.currentUser){
    const user=state.users.find(u=>u.email===state.currentUser.email);
    if(user){ user.puntaje=(user.puntaje||0)+score; user.partidas=(user.partidas||0)+1; user.lastActive=Date.now(); user.isPlaying=false; user.isPlayingQuiz=null; state.currentUser=user; }
    state.quizPlays.snake=(state.quizPlays.snake||0)+1;
    save(); pushFeed(`${state.currentUser.nick} jugó Serpiente BIFOR y obtuvo ${score} pts`, 'juego');
    setPlaying(state.currentUser.email,false);
  }
  document.getElementById('resultTitle').textContent='Serpiente terminada';
  document.getElementById('resultScore').textContent=score+' pts';
  document.getElementById('resultMessage').textContent=score ? `¡Buen trabajo! Sumaste ${score} puntos al ranking.` : 'Atrapa los puntos turquesa y evita chocar para sumar.';
...
function finishQuiz(){
  ...
  document.getElementById('resultTitle').textContent='Quiz Completado!';
...
document.getElementById('salirSnake')?.addEventListener('click',()=>{
  if(state.currentUser) setPlaying(state.currentUser.email,false);
  stopSnake();
  renderJuegos();
});
...
document.getElementById('snakeCanvas')?.addEventListener('touchstart', event=>{
  if(!state.snake) return;
  const touch=event.changedTouches[0];
  snakeTouchStart={x:touch.clientX,y:touch.clientY};
  event.preventDefault();
}, {passive:false});
document.getElementById('snakeCanvas')?.addEventListener('touchend', event=>{
  if(!state.snake || !snakeTouchStart) return;
  const touch=event.changedTouches[0];
  const dx=touch.clientX-snakeTouchStart.x, dy=touch.clientY-snakeTouchStart.y;
  snakeTouchStart=null;
  if(Math.max(Math.abs(dx),Math.abs(dy))<18) return;
  setSnakeDirection(Math.abs(dx)>Math.abs(dy) ? (dx>0?'right':'left') : (dy>0?'down':'up'));
  event.preventDefault();
}, {passive:false});
window.addEventListener('keydown', event=>{
  const keys={ArrowUp:'up',ArrowDown:'down',ArrowLeft:'left',ArrowRight:'right'};
  if(keys[event.key] && state.snake){ event.preventDefault(); setSnakeDirection(keys[event.key]); }
});
